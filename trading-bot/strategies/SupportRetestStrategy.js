// File: /trading-bot/strategies/SupportRetestStrategy.js
...
class SupportRetestStrategy extends EventEmitter {
    constructor(masterController, config, instrumentLoader, telegramService, positionManager, dataFetcher) {
        ...
        if (!config || !config.underlying || !config.token || !config.exchange) {
            console.error(`❌ [${config.underlying || 'Unknown'}] Invalid configuration provided to SupportRetestStrategy. Missing underlying, token, or exchange. Strategy will not initialize.`);
        ...
        this.params = config.support_retest_params || {};
    ...
    processData(tick) {
        ...
        if (numericTickToken === this.underlying.token) {
            ...
            for (const support of this.supportLevels) {
                if (this.tradedLevels.has(support.level) && (now.valueOf() - this.tradedLevels.get(support.level) < this.tradeCooldown)) continue;
                if (this.watchedLevels.has(support.level) && this.watchedLevels.get(support.level).candleTime.isSame(currentCandleTime)) continue;

                if (this.underlying.ltp <= support.level * 1.001 && this.underlying.ltp >= support.level) {
                    this.watchedLevels.set(support.level, { candleTime: currentCandleTime });
                    this.telegramService.sendMessage(`🔔 *Support Touch Alert for ${this.underlying.symbol}*\n\n*Support Level:* \`${support.level.toFixed(2)}\`\n*Action:* Waiting for 15-min candle to close for confirmation.`);
                }
            }
        }
        ...
        try {
            ...
            for (const [level] of levelsToCheck) {
                ...
                if (isBullish && isAboveSupport && openedAboveSupport && testedSupport) {
                    ...
                    this.telegramService.sendAlertMessage(`✅ *Support Confirmed for ${this.underlying.symbol}*\n\n*Level:* \`${level.toFixed(2)}\`\n*Confirmation:* Bullish candle tested, opened, and closed above support.`);
                    ...
                    this.telegramService.sendMessage(`❌ *Support REJECTED for ${this.underlying.symbol}*\n\n*Level:* \`${level.toFixed(2)}\`\n*Reason:* Confirmation candle ${reason}.`);
                }
                // ---- END MODIFICATION ----
    ...
    async executeTradeSetup(underlyingCandle) {
        try {
            ...
            const callOption = options.find(o => o.symbol.endsWith('CE'));

            if (!callOption) {
                console.warn(`[${this.underlying.symbol}] Could not find ATM Call Option on the specified exchange.`);
                return;
            }
            ...
            const volumeCheckHistory = await this.getOptionCandlesForVolumeCheck(callOption, underlyingCandle);
            if (!volumeCheckHistory || volumeCheckHistory.length < 2) {
                const message = `⚠️ *Trade Setup SKIPPED (CE) for ${this.underlying.symbol}*\n\n*Symbol:* \`${callOption.symbol}\`\n*Reason:* Not enough historical data for volume check.`;
                console.log(`[${this.underlying.symbol}] Trade setup for ${callOption.symbol} skipped: ${message}`);
            ...
            if (volumeConditionMet) {
                const confirmationMessage = `✅ *Volume Confirmed (CE) for ${this.underlying.symbol}*\n\n*Symbol:* \`${callOption.symbol}\`\n*Reason:* Volume condition met.\n  - Current Vol: \`${currentVolume}\`\n  - Previous Vol: \`${previousVolume}\` (${previousIsGreen ? 'Green' : 'Red'})`;
                console.log(`[${this.underlying.symbol}] Volume confirmed for ${callOption.symbol}.`);
                this.telegramService.sendMessage(confirmationMessage);
            } else {
                const message = `❌ *Trade Rejected (CE) for ${this.underlying.symbol}*\n\n*Symbol:* \`${callOption.symbol}\`\n*Reason:* Volume condition not met.\n  - Current Vol: \`${currentVolume}\`\n  - Previous Vol: \`${previousVolume}\` (${previousIsGreen ? 'Green' : 'Red'})`;
                console.log(`[${this.underlying.symbol}] Trade setup for ${callOption.symbol} rejected. ${message}`);
                this.telegramService.sendMessage(message);
                return;
            }
            */
            ...
            if (this.config.rsiSettings && this.config.rsiSettings.enabled) {
                const rsiValue = await this.getOptionRsiValue(callOption);
            ...
            const atrValue = await this.getOptionAtrValue(callOption);
            if (atrValue === null) {
            ...
            const optionCandle = await this.getOptionCandle(callOption, underlyingCandle);
            if (!optionCandle) {
                this.telegramService.sendMessage(`⚠️ *Trade Setup SKIPPED (CE) for ${this.underlying.symbol}*\n\n*Symbol:* \`${callOption.symbol}\`\n*Reason:* Could not fetch option candle data after multiple retries.`);
                return;
            }

            const targetBuyPrice = optionCandle[3]; 
            const numericToken = Number(callOption.token);

            this.telegramService.sendMessage(`🚀 *Trade Setup (CE) for ${this.underlying.symbol}*\n\n*Symbol:* \`${callOption.symbol}\`\n*Entry Target:* \`${targetBuyPrice.toFixed(2)}\`\n*Option ATR:* \`${atrValue.toFixed(2)}\`\n*Action:* Strategy is now watching for entry.`);

            this.watchedOptions.set(numericToken, {
                instrument: { ...callOption, token: numericToken, lotsize: Number(callOption.lotsize) },
    ...
    async initialize() {
        ...
        console.log(`[${this.underlying.symbol}] SupportRetestStrategy Initialized. Current candle starts at: ${this.lastCandleTime.format('HH:mm')}`);
    }
...
module.exports = SupportRetestStrategy;
