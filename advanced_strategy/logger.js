...
class Logger {
    constructor(config) {
        this.tradeLogStream = fs.createWriteStream(config.logFiles.tradeLogCsv, { flags: 'a' });
        this.errorLogStream = fs.createWriteStream(config.logFiles.errorLog, { flags: 'a' });
        this.debugMode = config.debugMode || false;

        // Write CSV Headers if file is new/empty
        if (fs.statSync(config.logFiles.tradeLogCsv).size === 0) {
