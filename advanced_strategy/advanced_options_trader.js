...
const SupportResistance = require('./sr_levels.js');
...
class AdvancedOptionsTrader {
    ...
    calculateIndicators(stock) {
        ...
        if (stock.candles.length > 10) {
            stock.srLevels = SupportResistance.detectLevels(
                stock.candles,
                this.config.srParameters.sensitivity
            );

            this.logger.debug(`Detected ${stock.srLevels.length} S/R levels for ${stock.symbol}`);
        } else {
            stock.srLevels = [];
        }
        // Bollinger Bands
    ...
    processData(data) { // Called by MasterController on LTP update
        ...
        if (this.config.srParameters.enabled && stock.srLevels.length > 0) {
            const relevantLevel = stock.srLevels.reduce((closest, level) => {
                if (!closest || Math.abs(level.level - ltp) < Math.abs(closest.level - ltp)) {
                    return level;
                }
                return closest;
            }, null);

            if (relevantLevel && Math.abs(relevantLevel.level - ltp) < this.config.srParameters.sensitivity) {
                // Resistance Breakout - Buy CE
                if (relevantLevel.type === 'resistance' && ltp > relevantLevel.level) {
                    const breakoutConfirmed = this.confirmBreakout(stock, relevantLevel, 'resistance');
                    if (breakoutConfirmed) {
                        this.executeBuy(stock, ltp, "Resistance Breakout");
                    }
                }

                // Support Breakout - Buy PE
                if (relevantLevel.type === 'support' && ltp < relevantLevel.level) {
                    const breakdownConfirmed = this.confirmBreakout(stock, relevantLevel, 'support');
                    if (breakdownConfirmed) {
                        this.executeBuy(stock, ltp, "Support Breakdown");
                    }
                }
            }
        }
    }
    confirmBreakout(stock, level, type) {
        const recentCandles = stock.candles.slice(-this.config.srParameters.breakoutConfirmationCandles);

        if (type === 'resistance') {
            return recentCandles.every(c => c.close > level.level);
        } else {
            return recentCandles.every(c => c.close < level.level);
        }
    }
    ...
    async executeBuy(stock, price, reason) {
        ...
        if (this.config.srParameters.enabled) {
            // Find nearest opposite level for TP
            const oppositeType = stock.option_type === 'CE' ? 'resistance' : 'support';
            const targetLevel = stock.srLevels
                .filter(l => l.type === oppositeType)
                .sort((a, b) =>
                    stock.option_type === 'CE' ? a.level - b.level : b.level - a.level
                )[0];

            if (targetLevel) {
                newPosition.tpPrice = targetLevel.level;
                this.logger.debug(`TP set at S/R level: ${targetLevel.level} for ${stock.symbol}`);
            }
        }
    ...
    setupTelegramCommands() {
        ...
        statusMsg += `S/R Levels:\n`;
        stock.srLevels.slice(-3).forEach(level => {
            statusMsg += `  - ${level.type.toUpperCase()} @${level.level.toFixed(2)} (${level.strength}x)\n`;
        });
