...
class AdvancedOptionsTrader {
    ...
    async fetchAllHistoricalData() {
        ...
        // Implementation example - replace with actual API calls
        for (const stock of this.stocks) {
            try {
                // Simulated historical data fetch
                const historicalData = await this.smart_api.getHistoricalData({
                    token: stock.token,
                    interval: this.config.tradingParameters.candleIntervalMinutes,
                    days: this.config.tradingParameters.historicalDataDays
                });

                stock.candles = historicalData.map(c => ({
                    timestamp: moment.unix(c[0]).tz("Asia/Kolkata"),
                    open: parseFloat(c[1]),
                    high: parseFloat(c[2]),
                    low: parseFloat(c[3]),
                ...
                this.logger.error(`Failed to fetch historical data for ${stock.symbol}:`, error);
                this.logger.error(`Failed to fetch historical data for ${stock.symbol}: Message: ${error.message}, Stack: ${error.stack}, API Response: ${error.response ? JSON.stringify(error.response.data) : 'N/A'}`);
            }
        }
    ...
    loadMajorSRLevels() {
        ...
        // Load NIFTY S/R levels
        try {
            const niftyData = fs.readFileSync(path.join(__dirname, 'nifty_sr_levels.csv'), 'utf-8');
            const niftyLevels = niftyData.split('\n').slice(1) // Skip header
                .map(line => parseFloat(line.split(',')[0]))
                .filter(level => !isNaN(level));
            this.majorSR.set('NIFTY', niftyLevels); // Assuming name is 'NIFTY'
            this.logger.info(`Loaded ${niftyLevels.length} major S/R levels for NIFTY.`);
        } catch (e) {
            this.logger.error("Failed to load Nifty S/R levels:", e.message);
        }

        // Load BANKNIFTY S/R levels
        try {
            const bankNiftyData = fs.readFileSync(path.join(__dirname, 'banknifty_sr_levels.csv'), 'utf-8');
            const bankNiftyLevels = bankNiftyData.split('\n').slice(1) // Skip header
                .map(line => parseFloat(line.split(',')[0]))
                .filter(level => !isNaN(level));
            this.majorSR.set('BANKNIFTY', bankNiftyLevels); // Assuming name is 'BANKNIFTY'
            this.logger.info(`Loaded ${bankNiftyLevels.length} major S/R levels for BANKNIFTY.`);
        } catch (e) {
            this.logger.error("Failed to load Bank Nifty S/R levels:", e.message);
        }
    }

    async processData(data) { // Changed to async for ML prediction
        ...
        // Validate indicators
        this.logger.debug(`[ProcessData Check - ${stock.symbol}] LTP: ${ltp}, BB: ${JSON.stringify(stock.bb)}, RSI: ${stock.rsi}, ATR: ${stock.atr}, Support: ${stock.support}, Resistance: ${stock.resistance}, OptionType: ${stock.option_type}`);

        if (!stock.bb || stock.rsi === null || stock.atr === null ||
            !stock.option_type || stock.support === null || stock.resistance === null) {
            this.logger.warn(`[ProcessData - ${stock.symbol}] Skipping trade check due to missing indicators/data. BB: ${!!stock.bb}, RSI: ${stock.rsi !== null}, ATR: ${stock.atr !== null}, S: ${stock.support !== null}, R: ${stock.resistance !== null}`);
            return;
        }
            ...
            if (stock.option_type === "CE") {
                if (ltp > stock.bb.upper && stock.rsi > this.config.tradingParameters.rsi.callBuyThreshold) {
                    if (ltp < stock.resistance * (1 - srBuffer)) {
                        ...
                        this.logger.debug(`[Check] <span class="math-inline">\{stock\.symbol\}\: LTP\=</span>{ltp.toFixed(2)}, BB_Upper=<span class="math-inline">\{stock\.bb?\.upper\.toFixed\(2\)\}, RSI\=</span>{stock.rsi?.toFixed(2)}, S=<span class="math-inline">\{stock\.support?\.toFixed\(2\)\}, R\=</span>{stock.resistance?.toFixed(2)}, OI=<span class="math-inline">\{oiSignal\}, ML\=</span>{mlPrediction}`);

                        if (oiConfirm && mlConfirm) {
                            this.executeBuy(stock, ltp, `BB_RSI_CE (OI:${oiSignal}, ML:${mlPrediction})`);
                        } else {
                            this.logger.debug(`CE Entry ${stock.symbol} skipped: OI=${oiSignal}, ML=${mlPrediction}`);
                        }
                    } else {
                        this.logger.debug(`CE Entry skipped for ${stock.symbol}: Near resistance (R:${stock.resistance.toFixed(2)} LTP:${ltp.toFixed(2)})`);
                    }
                }
                ...
                if (ltp < stock.bb.lower && stock.rsi < this.config.tradingParameters.rsi.putBuyThreshold) {
                    if (ltp > stock.support * (1 + srBuffer)) {
                        ...
                        if (oiConfirm && mlConfirm) {
                            ...
                            this.logger.debug(`PE Entry ${stock.symbol} skipped: OI=${oiSignal}, ML=${mlPrediction}`);
                        }
                    } else {
                        this.logger.debug(`PE Entry skipped for ${stock.symbol}: Near support (S:${stock.support.toFixed(2)} LTP:${ltp.toFixed(2)})`);
                    }
                }
            }
        }
    ...
    async executeBuy(stock, price, reason) {
        ...
        const atrVal = stock.atr || price * 0.01;
        let slPrice, tpPrice;

        // Calculate initial SL/TP based on option type
        if (stock.option_type === "CE") {
        ...
        const newPosition = {
            ...
            highestPriceSinceBuy: stock.option_type === "CE" ? price : null,
            lowestPriceSinceBuy: stock.option_type === "PE" ? price : null,
