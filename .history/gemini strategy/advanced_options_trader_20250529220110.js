...
class AdvancedOptionsTrader {
    ...
    async initialize() {
        ...
        // 2. Load Stocks (this.stocks is already loaded in constructor)
        if (this.stocks.length === 0) {
            this.logger.warn("⚠️ No stocks loaded from updated_options.json. Ensure test.js has run successfully and populated this file.");
        } else {
            this.logger.info(`✅ Loaded ${this.stocks.length} options/instruments from updated_options.json`);
        }
    ...
    async processData(data) {
        ...
        let majorUnderlyingSRData = null;
        let underlyingKey = null;
        // Derive underlyingKey (ensure this logic correctly maps option to its S/R data key)
        // This is a simplified example; you might need a more robust way to get the underlying.
        const stockSymbolUpper = stock.symbol.toUpperCase();
        if (stockSymbolUpper.includes("NIFTY")) underlyingKey = "NIFTY";
        else if (stockSymbolUpper.includes("BANKNIFTY")) underlyingKey = "BANKNIFTY";
        else { // Try to match with configured stock names for S/R
            const stockSRConfigs = this.config.majorStockSR || {};
            for (const nameInConfig in stockSRConfigs) {
                if (stockSymbolUpper.startsWith(nameInConfig.toUpperCase())) { // e.g. RELIANCE合约 starts with RELIANCE
                    underlyingKey = nameInConfig;
                    break;
                }
            }
        }
        ...
        if (underlyingKey) {
            majorUnderlyingSRData = this.majorSR.get(underlyingKey);
        }

        if (majorUnderlyingSRData) {
            this.logger.debug(`[<span class="math-inline">\{stock\.symbol\}\] Major Underlying \(</span>{underlyingKey}) Supports: ${majorUnderlyingSRData.supports.join(', ')}`);
            this.logger.debug(`[<span class="math-inline">\{stock\.symbol\}\] Major Underlying \(</span>{underlyingKey}) Resistances: ${majorUnderlyingSRData.resistances.join(', ')}`);
            // Use majorUnderlyingSRData.supports and majorUnderlyingSRData.resistances in your logic
        }
