...
class PreMarketAnalyzer {
    ...
    async processUnderlying(underlying) {
        ...
        // Use the new S/R detection with parameters from config
        const { sensitivity_percent, strength_threshold } = this.config.sr_calculation_parameters;
        underlying.sr_levels = SrCalculator.detectLevels(
            underlying.candles,
            sensitivity_percent,
            strength_threshold
        );
        this.logger.info(`📊 Found ${underlying.sr_levels.length} significant S/R zones for ${underlying.symbol}:`);
        underlying.sr_levels.forEach(zone => {
            this.logger.info(`   - ${zone.type} at ${zone.level.toFixed(2)} (strength: ${zone.strength})`);
        });
        if (underlying.sr_levels.length === 0) {
            this.logger.warn(`No significant S/R zones found for ${underlying.symbol}, cannot proceed.`);
            return setupsForThisUnderlying;
        }
        ...
        for (const option of selectedOptions) {
            ...
            if (option.candles && option.candles.length > 0) {
                // Calculate S/R on the option chart itself
    ...
    async getHistoricalData(symbol, exchange, token, histConfig, isUnderlying) {
        ...
        if (this.config.save_candles_to_csv && candles.length > 0) {
            const subFolder = isUnderlying ?
                `underlying_candles_${interval.replace('minute', 'min')}` :
                `option_candles_${interval.replace('minute', 'min')}`;
    ...
    async selectOptions(underlyingBaseSymbol, atmPrice) {
        ...
        const strikeStep = underlyingBaseSymbol.includes('BANKNIFTY') ?
            100 :
            (underlyingBaseSymbol.includes('NIFTY') ? 50 : 20);
        const atmStrike = Math.round(atmPrice / strikeStep) * strikeStep;
        const strikesToFetch = Array.from(
            { length: strikes_from_atm * 2 + 1 },
            (_, i) => atmStrike + (i - strikes_from_atm) * strikeStep
        );
    ...
    async getTargetExpiry(underlyingBaseSymbol) {
        const isIndex = ['NIFTY', 'BANKNIFTY'].includes(underlyingBaseSymbol);
        return this.api.instrumentManager.findNearestExpiry(underlyingBaseSymbol, isIndex);
    }

    // Then update selectOptions method:
    async selectOptions(underlyingBaseSymbol, atmPrice) {
        const { strikes_from_atm } = this.config.options_selection_criteria;
        const strikeStep = underlyingBaseSymbol.includes('BANKNIFTY') ?
            100 :
        ...
        // Get expiry using the new method
        const expiry = await this.getTargetExpiry(underlyingBaseSymbol);
        if (!expiry) {
            this.logger.error(`No valid expiry found for ${underlyingBaseSymbol}`);
            return [];
        }

        return this.api.fetchOptionsDetailsByStrikes(underlyingBaseSymbol, strikesToFetch, expiry);
    }
}
