...
class ApiService {
    ...
    /**
     * *** Final version: This single function now robustly returns both the ATM price and the candle data used to find it. ***
     */
    async getAtmPriceAndCandles(symbol, exchange, token) {
        let candles = [];
        // 1. Primary Method: Get last few daily candles.
        try {
            this.logger.info(`Attempting to derive ATM price for ${symbol} via recent daily candles...`);
            const { from_date, to_date } = this.getSafeDateRange(30); // Use 30 days for a decent candle history
            ...
            if (candles.length > 0) {
                ...
                return { price: lastClose, candles: candles }; // Return both price and candles
            }
        } catch (error) {
        ...
        if (instrument?.close) {
            ...
            // Cannot return candles here, so return null for candles
