...
class PreMarketAnalyzer {
    ...
    async processUnderlying(underlying) {
        ...
        const srLevels = SrCalculator.detectLevels(srCandles, { reactionLookback, levelsToReturn });
        ...
        for (const option of selectedOptions) {
            ...
            if (option.candles && option.candles.length > 0) {
                ...
                option.sr_levels = SrCalculator.detectLevels(option.candles, { reactionLookback: 3, levelsToReturn: 4 });
                const setups = this.tradeIdentifier.identify(underlying, option, atmPrice);
                if (setups.length > 0) {
