...
class PreMarketAnalyzer {
    ...
    async processUnderlying(underlying, allSetups) {
        ...
        // ** THE FIX IS HERE **
        // Pass the token directly to the data fetcher
        ...
        const selectedOptions = await this.selectOptions(underlying, ltpRes.ltp);

        for (const option of selectedOptions) {
            option.candles = await this.getHistoricalData(option.tradingsymbol, this.config.options_selection_criteria.expiry_type === 'WEEKLY' ? 'NFO' : 'NFO', option.token, this.config.option_historical_config, false);
    ...
    async getHistoricalData(symbol, exchange, token, histConfig, isUnderlying) {
        ...
        // Optional: Add logic to check if CSV is recent enough
        ...
        const candles = await this.api.fetchHistoricalCandlesAPI({ tradingsymbol: symbol, exchange, symboltoken: token, interval: histConfig.interval, from_date: from, to_date: to });
        if (candles.length > 0) CsvHandler.save(csvPath, candles);
        return candles;
    }

    async selectOptions(underlying, atmPrice) {
        const { strikes_from_atm } = this.config.options_selection_criteria;
        const strikeStep = underlying.symbol.includes('BANKNIFTY') ? 100 : (underlying.symbol.includes('NIFTY') ? 50 : 20);
        ...
        return await this.api.fetchOptionsDetailsByStrikes(underlying.symbol, strikesToFetch, expiry);
    }
