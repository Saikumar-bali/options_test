...
class PreMarketAnalyzer {
    ...
    // The rest of this file's code is correct and remains unchanged from the previous version.
    // No further modifications are needed here as the changes are handled in the ApiService.
    async run() {
    ...
    async processUnderlying(underlying, allSetups) {
        ...
        if (underlying.candles.length === 0) { this.logger.warn(`Could not fetch candles for ${underlying.symbol}. Skipping.`); return; }
        ...
        // *** FIX: Call the new robust getAtmPrice method ***
        const atmPrice = await this.api.getAtmPrice(underlying.symbol, underlying.underlying_segment, underlying.token);
        if (!atmPrice) {
            this.logger.warn(`Could not fetch ATM price for ${underlying.symbol}. Skipping options search.`);
            return;
        }
    ...
    generateOutputFile(setups) {
        ...
        const outputList = setups.map(s => ({
            symbol: s.tradingsymbol || s.symbol, token: s.token, exch_seg: s.exch_seg, lotsize: s.lotsize, option_type: s.instrument_type, expiry: s.expiry_date, strike: s.strike_price, underlying: s.underlying_symbol, underlying_sr_level: s.uSr, option_sr_level: s.oSr, trade_setup_reason: s.reason, recommended_direction: s.direction
        }));
