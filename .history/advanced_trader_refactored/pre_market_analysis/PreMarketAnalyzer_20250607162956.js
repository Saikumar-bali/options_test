...
class PreMarketAnalyzer {
    ...
    findCorrectExpiryForSymbol(underlyingSymbol) {
        ...
        try {
            ...
            if (!allExpiries || allExpiries.length === 0) {
                this.logger.error(`No expiries found for ${baseSymbol}`);
                return null;
            }
            ...
            if (baseSymbol === 'NIFTY') {
                return futureExpiries[0].format('DDMMMYYYY').toUpperCase();
            } 
            else if (baseSymbol === 'BANKNIFTY') {
                // BANKNIFTY: Use weekly expiry (Thursday)
                const nextThursday = today.clone().day(4); // 4=Thursday
                if (today.day() > 4 || (today.day() === 4 && today.hour() >= 16)) {
                    nextThursday.add(1, 'weeks');
                }
                return nextThursday.format('DDMMMYYYY').toUpperCase();
            }
            else {
                // Stocks: Use monthly expiry (last Thursday)
                const monthEnd = today.clone().endOf('month');
                const lastThursday = monthEnd.day(4); // 4=Thursday
                if (lastThursday.date() > monthEnd.date()) {
                    lastThursday.subtract(1, 'week');
                }
                return lastThursday.format('DDMMMYYYY').toUpperCase();
            }
        } catch (error) {
            this.logger.error(`Error finding expiry for ${baseSymbol}: ${error.message}`);
            return null;
        }
    }
