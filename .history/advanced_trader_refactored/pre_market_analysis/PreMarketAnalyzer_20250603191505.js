...
const SrCalculator = require('../indicators/SupportResistance');
...
class PreMarketAnalyzer {
    ...
    async processUnderlying(underlying, allSetups) {
        ...
        // --- FINAL STREAMLINED LOGIC ---
        // 1. Get the ATM price and the candles used to derive it in a single, robust call.
        const { price: atmPrice, candles: underlyingCandles } = await this.api.getAtmPriceAndCandles(underlying.symbol, underlying.underlying_segment, underlying.token);

        if (!atmPrice || !underlyingCandles || underlyingCandles.length === 0) {
            this.logger.warn(`Could not determine ATM price or candle history for ${underlying.symbol}. Skipping.`);
            return;
        }
        underlying.candles = underlyingCandles; // Store the candles we successfully received.

        // 2. Calculate S/R levels using the candles we know we have.
        ...
        this.logger.info(`📊 Found ${underlying.sr_levels.length} S/R levels for ${underlying.symbol} from recent daily data.`);
        
        // 3. Select options based on the derived ATM price.
        ...
        for (const option of selectedOptions) {
            // 4. Fetch the option's own candle history for its S/R levels.
            option.candles = await this.getOptionCandleData(option.symbol, option.token);
    ...
    // Helper for option candles, as it uses a different config
    async getOptionCandleData(symbol, token) {
        this.logger.info(`Fetching fresh data for option: ${symbol}...`);
        const { from_date, to_date } = this.api.getSafeDateRange(this.config.option_historical_config.duration_days);
        const candles = await this.api.fetchHistoricalCandlesAPI({ 
            tradingsymbol: symbol, 
            exchange: 'NFO', 
            symboltoken: token, 
            interval: this.config.option_historical_config.interval, 
            from_date, 
            to_date 
        });
        // Optional: Save to CSV
        if (candles.length > 0) {
             const csvPath = path.join(this.config.data_store_path, 'option_candles_15min', `${symbol}.csv`);
    ...
    generateOutputFile(setups) {
        ...
        const outputList = setups.map(s => ({
            symbol: s.symbol, token: s.token, exch_seg: s.exch_seg, lotsize: s.lotsize, option_type: s.instrument_type, expiry: s.expiry_date, strike: s.strike, underlying: s.underlying_symbol, underlying_sr_level: s.uSr, option_sr_level: s.oSr, trade_setup_reason: s.reason, recommended_direction: s.direction
        }));
