...
class PreMarketAnalyzer {
    constructor(config, logger, masterController) {
        ...
        this.api.logger = logger; // Pass logger for debug prints
    ...
    async processUnderlying(underlying, allSetups) {
        ...
        // Use the base symbol (without -EQ) for finding options
    ...
    // Unchanged methods from previous response below
    ...
    generateOutputFile(setups) {
        ...
        const outputList = setups.map(s => ({ symbol: s.tradingsymbol, token: s.token, exch_seg: s.exch_seg, lotsize: s.lotsize, option_type: s.instrument_type, expiry: s.expiry_date, strike: s.strike_price, underlying: s.underlying_symbol, underlying_sr_level: s.uSr, option_sr_level: s.oSr, trade_setup_reason: s.reason, recommended_direction: s.direction }));
