...
class PreMarketAnalyzer {
    ...
    async processUnderlying(underlying) {
        ...
        if (!srLevels.support || !srLevels.resistance) {
            this.logger.warn(`Could not determine a clear support/resistance boundary for ${underlying.symbol}. Skipping.`);
            return setupsForThisUnderlying;
        }

        // Store the single best levels
        underlying.sr_levels = [srLevels.support, srLevels.resistance];

        this.logger.info(`📊 Key Levels for ${underlying.symbol}:`);
        this.logger.info(`  - Nearest Support: ${srLevels.support.level.toFixed(2)} (Reaction: ${srLevels.support.reaction.toFixed(2)})`);
        this.logger.info(`  - Nearest Resistance: ${srLevels.resistance.level.toFixed(2)} (Reaction: ${srLevels.resistance.reaction.toFixed(2)})`);

        const optionsBaseSymbol = underlying.symbol.replace('-EQ', '');
        // For simplicity, we can just select options around the ATM price
        ...
        for (const option of selectedOptions) {
            ...
            if (option.candles && option.candles.length > 0) {
                const setups = this.tradeIdentifier.identify(underlying, option); // trade_identifier will use the new S/R levels
                if (setups.length > 0) {
