...
class PreMarketAnalyzer {
    ...
    async processUnderlying(underlying) {
        ...
        if (!atmPrice) {
            ...
            return setupsForThisUnderlying; // Return empty array
        }
        this.logger.info(`[ATM] Derived ATM price for ${underlying.symbol} as ${atmPrice}.`);
        
        // 2. Get historical data for S/R analysis
        ...
        // 4. If S/R levels were found, proceed with options
    ...
    getTargetExpiry() {
        ...
        if(today.day() > 4 || (today.day() === 4 && today.hour() >= 16)) {
             expiryDay = today.day() === 5 ? 6 : 7; // Find next week's expiry
        }
