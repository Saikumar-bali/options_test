...
class ApiService {
    ...
    /**
     * *** FIX: Creates a highly robust date range, always ending on the last completed trading day. ***
     * This is the most reliable way to request historical data from trading APIs.
     */
    getSafeDateRange(durationDays) {
        let toDate = moment.tz("Asia/Kolkata");

        // If today is a weekend or a weekday before market close, we need to find the last valid trading day.
        if (toDate.day() === 0) { // Sunday
            toDate.subtract(2, 'days');
        } else if (toDate.day() === 6) { // Saturday
            toDate.subtract(1, 'days');
        } else if (toDate.hour() < 16) { // It's a weekday, but market might not have generated the full EOD candle yet.
             toDate.subtract(1, 'days');
             // After subtracting, check again if we landed on a weekend.
             if(toDate.day() === 0) toDate.subtract(2, 'days');
             if(toDate.day() === 6) toDate.subtract(1, 'days');
        }
    ...
    async getAtmPrice(symbol, exchange, token) {
        ...
        try {
            this.logger.info(`Attempting to derive ATM price for ${symbol} via last daily candle...`);
            ...
            if (candles.length > 0) {
                ...
                this.logger.info(`[LTP] Derived ATM price for ${symbol} as ${lastClose}.`);
                return lastClose;
            }
        } catch (error) {
        ...
        if (instrument?.close) {
            ...
            this.logger.info(`[LTP] Derived ATM price for ${symbol} as ${lastClose} from Scrip Master.`);
            return lastClose;
        }
