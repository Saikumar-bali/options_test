...
class PreMarketAnalyzer {
    ...
    async processUnderlying(underlying, allSetups) {
        ...
        this.logger.info(`Fetching ${this.config.underlying_historical_config.interval} candles for S/R analysis for ${underlying.symbol}...`);
        ...
        if (!srCandles || srCandles.length === 0) {
            this.logger.warn(`No S/R candles found for ${underlying.symbol}. Skipping option analysis for this underlying.`);
            return;
        }
        ...
        this.logger.info(`📊 Found ${underlying.sr_levels.length} S/R levels for ${underlying.symbol} from ${this.config.underlying_historical_config.interval} data.`);
    ...
    setupDataStore() {
        ...
        let underlyingSubFolder = `underlying_candles_${underlyingInterval.replace('minute','min')}`;
        let optionSubFolder = `option_candles_${optionInterval.replace('minute','min')}`;
