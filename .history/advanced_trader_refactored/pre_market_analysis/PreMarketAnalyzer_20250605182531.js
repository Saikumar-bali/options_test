...
class PreMarketAnalyzer {
    ...
    mergeCandleData(existingCandles, newCandles) {
        const candleMap = new Map();
        // Add existing candles to the map first
        existingCandles.forEach(candle => candleMap.set(candle.timestamp, candle));
        // Add new candles, overwriting any duplicates (ensuring we have the latest)
        newCandles.forEach(candle => candleMap.set(candle.timestamp, candle));
        
        // Convert map back to an array and sort by timestamp
    ...
    getTargetExpiry() {
        ...
        if (today.day() > 4 || (today.day() === 4 && today.hour() >= 16)) {
            // If past Thursday 4 PM, find next Thursday
