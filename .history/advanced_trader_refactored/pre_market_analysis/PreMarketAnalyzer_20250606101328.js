...
class PreMarketAnalyzer {
    ...
    async processUnderlying(underlying) {
        ...
        if (!atmPrice) return setupsForThisUnderlying;
        ...
        if (!srCandles || srCandles.length === 0) return setupsForThisUnderlying;
        ...
        const srLevels = SrCalculator.detectLevels(underlying.candles, { reactionLookback, levelsToReturn });
        underlying.sr_levels = [...srLevels.supports, ...srLevels.resistances]; // Combine for later use

        this.logger.info(`📊 Found Top S/R levels for ${underlying.symbol}:`);
        srLevels.supports.forEach(s => this.logger.info(`  - Explosive Support at ${s.level.toFixed(2)} (Reaction: ${s.reaction.toFixed(2)})`));
        srLevels.resistances.forEach(r => this.logger.info(`  - Explosive Resistance at ${r.level.toFixed(2)} (Reaction: ${r.reaction.toFixed(2)})`));
    ...
    getTargetExpiry() {
        ...
        // If today is past Thursday's market close, find next week's Thursday
        if (today.day() > expiryDay || (today.day() === expiryDay && today.hour() >= 16)) {
            // Add days to get to the *next* Thursday
            const daysToAdd = ((expiryDay + 7) - today.day()) % 7;
            // if today is Thursday, adding 0 days won't work, so add 7
