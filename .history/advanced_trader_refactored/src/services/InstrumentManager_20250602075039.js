...
// Define SCRIP_MASTER_PATH relative to this file's location or an absolute path
// Assuming data_store is at ../pre_market_analysis/data_store/ relative to this /src/services/ location
...
class InstrumentManager {
    ...
    async initialize() {
        try {
            ...
            // Optional: Check file age and download only if older than a day
            ...
            if (fs.existsSync(SCRIP_MASTER_PATH)) {
                ...
                if (moment().diff(lastModified, 'hours') < 24) { // If less than 24 hours old
            ...
            if (Array.isArray(scripMasterData)) {
                ...
                this.scripMaster = []; // Ensure it's an array to prevent errors
            }

        } catch (error) {
            this.logger.error("Failed to download or load Scrip Master.", error.message);
            if (fs.existsSync(SCRIP_MASTER_PATH) && !this.scripMaster) { // If download failed but local exists and not loaded
                try {
                    ...
                    this.scripMaster = []; // Critical error, ensure scripMaster is an array
                }
            } else if (!this.scripMaster) {
                 this.scripMaster = []; // Ensure scripMaster is an array if all fails
            }
        }
    }
    ...
    buildInstrumentMaps() {
        ...
        for (const instrument of this.scripMaster) {
            ...
            if (instrument.symbol && instrument.exch_seg) {
                // For NIFTY/BANKNIFTY, the symbol in master might be "NIFTY" not "NIFTY INDEX"
                const keySymbol = instrument.name === "NIFTY" ? "NIFTY" : 
                                  instrument.name === "BANKNIFTY" ? "BANKNIFTY" : 
                                  instrument.symbol; // Use symbol for stocks like "RELIANCE-EQ"
    ...
    getInstrumentDetails(symbol, exchange) {
        ...
        // Handle common variations for index symbols if necessary
        let searchSymbol = symbol;
        if (exchange === 'NSE' && (symbol === 'NIFTY' || symbol === 'BANKNIFTY')) {
             // Scrip master 'name' field is often NIFTY/BANKNIFTY for indices, 'symbol' can vary.
             // The map building uses name for these indices.
        } else if (exchange === 'NSE' && symbol.endsWith('-EQ')) {
            // searchSymbol = symbol; // Already correct for stocks like RELIANCE-EQ
        }
        
        const key = `${searchSymbol}_${exchange}`;
    ...
    findOption(underlyingName, strikePrice, expiryDate, optionType) {
        ...
        // expiryDate is expected as 'YYYY-MM-DD'
        ...
        return this.scripMaster.find(item => 
            ...
            item.exch_seg === 'NFO' && // Options are in NFO
            item.expiry.toUpperCase() === formattedExpiry &&
            parseFloat(item.strike) * 100 === strikePriceNum * 100 && // Compare strike (master has it in paisa or actual value)
