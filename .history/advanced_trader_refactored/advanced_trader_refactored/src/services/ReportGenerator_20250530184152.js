...
class ReportGenerator {
    ...
    buildReportText(reportDate, closedTrades, openPositions, realizedPnl) {
        ...
        const status = riskManager.isTradingHalted() ? (riskManager.isManuallyHalted() ? 'MANUALLY HALTED 🔴' : 'HALTED (Limit) ⚠️') : 'ACTIVE 🟢';
        text += `*Trading Status:* ${status}\n\n`;

        text += "*--- CLOSED TRADES ---*\n";
        if (closedTrades.length > 0) {
            closedTrades.forEach(t => {
                if (t.action.toUpperCase() === 'SELL') {
                    text += `_${moment(t.timestamp).format("HH:mm:ss")}_ ${t.symbol} | SELL @${t.price.toFixed(2)} | P&L: *₹${t.pnl.toFixed(2)}*\n`;
                }
            });
        } else {
        ...
        if (openPositions.length > 0) {
             openPositions.forEach(pos => {
                ...
                unrealizedPnl += pnl;
                text += `${pos.symbol} | BUY @${pos.buyPrice.toFixed(2)} | LTP @${ltp.toFixed(2)} | Unrealized P&L: *₹${pnl.toFixed(2)}*\n`;
            });
        } else {
        ...
        return text.replace(/\*/g, ''); // Remove markdown for .txt file
    }
}
